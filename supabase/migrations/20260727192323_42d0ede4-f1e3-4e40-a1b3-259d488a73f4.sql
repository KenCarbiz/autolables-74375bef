...
CREATE INDEX IF NOT EXISTS idx_vehicle_lifecycle_tenant_state ON public.vehicle_lifecycle (tenant_id, state);
CREATE INDEX IF NOT EXISTS idx_vehicle_lifecycle_vin ON public.vehicle_lifecycle (tenant_id, vin);
...
WHERE si.status = 'pending'
  AND NOT EXISTS (SELECT 1 FROM public.vehicle_listings v WHERE v.tenant_id = si.tenant_id AND v.vin = si.vin);

INSERT INTO public.vehicle_lifecycle (tenant_id, vehicle_id, vin, state, authorized_at, gate_reason)
SELECT v.tenant_id, v.id, upper(v.vin), 'AUTHORIZED_FOR_GET_READY', now(), 'grandfathered at lifecycle rollout'
