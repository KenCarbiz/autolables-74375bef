-- ──────────────────────────────────────────────────────────────────────
-- Tier-gate the price-integrity enforcement.
--
-- The hard signing gate is a Compliance Pro (premium) capability. Make the
-- server enforcement conditional on the tenant having the feature ON
-- (dealer_profiles.settings.feature_price_verification = true). Default OFF, so
-- tenants who don't pay for it sign exactly as before — no price gate. This
-- also undoes the earlier blanket enforcement that blocked every tenant.
-- ──────────────────────────────────────────────────────────────────────
...
-- Trigger: only enforce when the tenant has price verification enabled.
...
-- record_customer_signing: same body as 20260618001000, but the addendum-branch
-- price guard now only fires for tenants with the feature ON.
...
BEGIN
  ...
  IF FOUND THEN
    -- Price-integrity gate — only for tenants entitled to it (Compliance Pro).
...
-- ── Admin provisioning: merge premium feature flags into a tenant's settings
-- so a super admin can turn the Compliance Pro stack on/off per tenant from the
-- platform grid (cross-tenant; dealer_profiles RLS would otherwise block it).
