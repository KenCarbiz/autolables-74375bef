...
--
-- Distinct from the K-208 (a State safety inspection signed by a writer/
-- manager): the PDI is the dealer's own final pre-delivery check, which a
-- technician OR a service writer may complete. Stored in its OWN table so it
-- never satisfies the K-208 finalize gate (which looks at safety_inspections).
-- Token-gated anon RPCs mirror the K-208 / detail / recon pattern.
-- ─────────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS public.pdi_signoffs (
  ...
  vin                text NOT NULL,
  ymm                text,
  checklist          jsonb NOT NULL DEFAULT '[]'::jsonb,   -- [{id,label,result,explanation}]
  result             text CHECK (result IN ('pass','fail')),
  notes              text,
  performer_name     text,
  performer_role     text,          -- technician | service_writer
  signature_data     text,
  content_hash       text,
  esign_consent      jsonb,
  customer_ip        text,
...
-- ── Is the PDI done for this vehicle? (anon, via the Get-Ready hub token) ────
...
-- ── Submit the PDI (anon, via the Get-Ready hub token; never consumes it) ────
