-- ──────────────────────────────────────────────────────────────────────
-- K-208 licensee certification — the two-party split.
--
-- The CT K-208 certification "must be signed by the licensee," a step distinct
-- from the technician who performs the physical inspection. Today the flow is
-- one row: pending (ingest) -> signed (technician). This adds an ADDITIVE
-- licensee-certification layer on top, so the derived lifecycle is:
--
--   pending                         -> awaiting inspection
--   status='signed', not certified  -> technician completed, awaiting licensee
--   licensee_certified_at set       -> licensee certified (A/B/C confirmed)
--
-- Per the owner's decision, ANY manager-level member may certify. This is
-- additive: it does NOT change the existing finalize gate (which still only
-- requires a technician-signed inspection), so no in-flight deal is blocked.
-- A dealer can later choose to require certification before delivery.
-- ──────────────────────────────────────────────────────────────────────
...
-- Certify the K-208 as the licensee. Requires a technician-completed inspection
-- (status='signed') and confirms the A/B/C result. Manager-gated.
...
BEGIN
  ...
  -- Any manager-level member of the tenant (owner decision), or a platform admin.
  SELECT EXISTS (
