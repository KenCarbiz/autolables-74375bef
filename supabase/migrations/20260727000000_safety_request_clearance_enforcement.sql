-- ──────────────────────────────────────────────────────────────────────
-- Phase 3a: safety authorization requests now gate delivery clearance.
--
-- service_requests.is_safety was written and displayed but read by no
-- gate: a manager DECLINING a safety-critical request silently unblocked
-- the vehicle, while the request UI promised "blocks delivery until
-- resolved". Per the approved spec, safety work cannot be dismissed by
-- declining the expense — the vehicle stays blocked until the repair is
-- authorized and verified, or the vehicle leaves retail (wholesale/remove).
--
--   * SAFETY_REQUEST_PENDING     — an open (pending/clarify) safety request.
--   * SAFETY_DECLINED_UNRESOLVED — a declined safety request whose linked
--     inspection item has not passed reinspection (or has no linked item).
--   * service_requests changes now recompute the stored clearance, so the
--     block appears and clears without waiting for a workspace visit.
-- ──────────────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_safety_pending boolean;
  v_safety_declined boolean;
...
BEGIN
  ...
  -- Safety authorization requests gate delivery. A declined safety request
  -- keeps blocking until its linked inspection item passes reinspection
  -- (unlinked requests block until the vehicle leaves retail).
  v_safety_pending := EXISTS (
    SELECT 1 FROM public.service_requests r
    WHERE r.tenant_id = p_tenant_id AND r.vin = v_vin
      AND r.is_safety AND r.status IN ('pending','clarify'));
  v_safety_declined := EXISTS (
    SELECT 1 FROM public.service_requests r
    WHERE r.tenant_id = p_tenant_id AND r.vin = v_vin
      AND r.is_safety AND r.status = 'declined'
      AND (r.inspection_item_id IS NULL OR EXISTS (
        SELECT 1 FROM public.safety_inspection_item_failures f
        WHERE f.id = r.inspection_item_id
  ...
  IF v_condition IS NOT NULL AND v_condition NOT IN ('used','cpo','certified') THEN
    ...
    END IF;
  ELSIF (v_latest_signed.id IS NOT NULL AND v_latest_signed.result = 'fail') OR v_open_failures
        OR v_safety_pending OR v_safety_declined THEN
    ...
    IF v_safety_pending THEN v_codes := array_append(v_codes, 'SAFETY_REQUEST_PENDING'); END IF;
    IF v_safety_declined THEN v_codes := array_append(v_codes, 'SAFETY_DECLINED_UNRESOLVED'); END IF;
...
-- Request changes recompute the stored clearance immediately.
CREATE OR REPLACE FUNCTION public.touch_clearance_on_request()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  BEGIN
    IF NEW.tenant_id IS NOT NULL AND coalesce(NEW.vin, '') <> '' THEN
      PERFORM public.recompute_delivery_clearance(NEW.tenant_id, NEW.vin);
    END IF;
  EXCEPTION WHEN OTHERS THEN NULL;
  END;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_clearance_touch_requests ON public.service_requests;
CREATE TRIGGER trg_clearance_touch_requests
  AFTER INSERT OR UPDATE ON public.service_requests
  FOR EACH ROW EXECUTE FUNCTION public.touch_clearance_on_request();
