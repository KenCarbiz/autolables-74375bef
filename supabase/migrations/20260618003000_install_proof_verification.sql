-- Verifiable install proofs (FTC): a product advertised as pre-installed and
-- non-removable must be substantiated by a real installer sign-off WITH a photo.
-- A bare row no longer "proves" an install — is_verified requires both a photo
-- and an installer signature. Otherwise the product must be reclassified to
-- customer-elected so the buyer may decline it.
...
-- A proof is verifiable only with both an image and an installer signature.
ALTER TABLE public.install_proofs
...
-- Overloaded RPC that captures the installer signature + IP and stamps
-- verified_at when a photo and signature are both present. The original
-- 8-arg overload remains for backward compatibility.
