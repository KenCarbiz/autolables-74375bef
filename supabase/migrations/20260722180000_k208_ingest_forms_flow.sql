-- ──────────────────────────────────────────────────────────────────────
-- Forms flow — Phase 1: document types + K-208 generated at ingest
--
-- Every used/CPO car must carry the FTC Buyers Guide AND the CT K-208 safety
-- inspection from the moment it lands. The Buyers Guide is already auto-drafted
-- at ingest (create_draft_buyers_guide). This adds the K-208 to that pipeline
-- and makes both first-class signable document types.
--
--   1. generated_documents CHECK += 'k208'  (the K-208 can be a document row
--      alongside window/addendum/passport/cpo_sheet/buyers_guide).
--   2. signed_document_archive CHECK += 'k208'  (fixes a gotcha: archive-pdf
--      already accepts doc_type 'k208' but the DB CHECK rejected it).
--   3. create_draft_safety_inspection(tenant, vin): pre-create a PENDING K-208
--      for every used/cpo/certified car at ingest, regardless of year/mileage/
--      value (CGS 14-62(g) has no thresholds). Idempotent.
--   4. submit_safety_inspection: complete the PENDING row in place (one K-208
--      row spans pending -> service-signed -> buyer-signed) instead of always
...
-- create_draft_safety_inspection(tenant, vin): the ingest-time K-208.
...
BEGIN
  ...
  -- Every used/CPO car needs a K-208 — no year/mileage/value threshold.
  IF v_condition NOT IN ('used','cpo','certified') THEN RETURN NULL; END IF;

  -- Idempotent: one K-208 record per vehicle (pending or already signed).
...
-- Redefine submit_safety_inspection to COMPLETE a pending K-208 in place (so the
-- ingest-created row carries through), falling back to INSERT for vehicles that
-- never got a pre-created draft. Everything else (token guards, consumption,
-- audit) is unchanged.
