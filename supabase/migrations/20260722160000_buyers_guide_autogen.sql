-- ──────────────────────────────────────────────────────────────────────
-- FTC Buyers Guide — auto-draft at ingest (16 CFR Part 455)
--
-- Every USED / CPO vehicle must carry a Buyers Guide. Until now the dealer
-- built it by hand each time. This drafts one automatically at ingest and files
-- it into the vehicle's generated_documents, so it is tracked from the moment a
-- car lands and can never be forgotten.
--
-- The row lands as a DRAFT: nothing is asserted to a customer until a manager
-- reviews and approves/publishes it in the existing Buyers Guide UI, which is
-- the authoritative renderer of the FTC canonical text and applies the full
-- warranty precedence (dealer warranty programs + per-store state). This RPC
-- only computes a STARTING warranty box from the two most load-bearing inputs —
-- the operating state's used-car statute and the dealer's configured default —
-- so the draft opens on the right answer for the common case. The box logic is
-- a faithful port of resolveBuyersGuideWarranty in src/lib/stateCompliance.ts;
-- that TS function remains the source of truth. needs_verification is always
-- true on the auto-draft.
-- ──────────────────────────────────────────────────────────────────────
...
ALTER TABLE public.generated_documents
  ...
  CHECK (document_type IN ('window','addendum','passport','cpo_sheet','buyers_guide'));
...
DECLARE
  ...
  v_price_n numeric := 0; v_miles_n int := 0;
...
BEGIN
  ...
  -- Only used / CPO cars need an FTC Buyers Guide.
  IF v_condition NOT IN ('used', 'cpo', 'certified') THEN RETURN NULL; END IF;

  -- Idempotent: one Buyers Guide per vehicle.
  ...
  -- Prefer the decoded year/make/model + mileage from the inventory record.
  ...
  -- Operating state + dealer default (resolveOperatingState precedence, minus
  -- the per-store rooftop state which the manager's review applies).
  ...
  -- Faithful port of resolveBuyersGuideWarranty (stateCompliance.ts).
  CASE v_state
    WHEN 'CT' THEN
    ...
    -- As-is-prohibited states → Implied Warranties Only front.
  ...
  -- Dealer default override (BuyersGuide.tsx precedence), applied only when the
  -- state didn't already force a box. The dealer-warranty-program lock is left
  -- to the client resolver at review time.
  ...
  INSERT INTO public.generated_documents (
    ...
    jsonb_build_object(
      ...
      'box', v_box,
      'forced', v_forced,
      'min_duration_days', v_days,
      'min_miles', v_mi,
      'min_pct', v_pct,
      'citation', v_citation,
      'needs_verification', true,
      'operating_state', v_state,
      'default_ftc_warranty', v_default,
...
REVOKE EXECUTE ON FUNCTION public.create_draft_buyers_guide(uuid, text) FROM public;
GRANT EXECUTE ON FUNCTION public.create_draft_buyers_guide(uuid, text) TO service_role;
