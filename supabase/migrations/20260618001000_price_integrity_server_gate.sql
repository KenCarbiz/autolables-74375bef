-- ──────────────────────────────────────────────────────────────────────
-- Price-integrity server gate (FTC advertised-price honor).
--
-- A customer may only sign once the all-in price (selling price + doc fee +
-- pre-installed, in-advertised items) reconciles to the scraped advertised
-- price for the VIN. The client computes the all-in total and writes it as
-- expected_total; this migration adds the verification state, an authoritative
-- verify RPC that compares it against the advertised_prices table server-side,
-- a guard inside record_customer_signing, and a defense-in-depth trigger so no
-- write path can flip an addendum to 'signed' while it is unverified.
-- ──────────────────────────────────────────────────────────────────────
...
-- Constrain the status vocabulary without failing on legacy rows.
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ──────────────────────────────────────────────────────────────────────
-- verify_addendum_price — authoritative setter. Reads the addendum's
-- client-computed expected_total and the latest advertised price for the VIN
-- (preferring the dealer's own website), then sets the verification state.
-- $50 default tolerance mirrors assessDrift / TOLERANCE_DOLLARS.
-- ──────────────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────────────
-- Defense-in-depth: no write path may flip an addendum to 'signed' while it
-- is not price_verified. Covers the authenticated desktop save path and any
-- legacy direct update. Customer remote signing is additionally guarded in
-- record_customer_signing below.
-- ──────────────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────────────
-- record_customer_signing — same body as 20260515191442, with a price-verified
-- guard added to the addendum branch so the remote token path cannot sign an
-- unverified deal even by hitting the URL directly.
-- ──────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF FOUND THEN
    -- Price-integrity gate: block signing unless the all-in price was verified
    -- against the advertised price for this VIN.
...
-- ──────────────────────────────────────────────────────────────────────
-- Widen get_addendum_by_token so the public signing pages can show the
-- verification state (and the selling price) instead of only failing at submit.
-- ──────────────────────────────────────────────────────────────────────
