-- ──────────────────────────────────────────────────────────────────────
-- Optional finalize gate: require K-208 LICENSEE CERTIFICATION before signing.
--
-- 20260723000000 added the licensee-certification step (a manager confirms the
-- A/B/C result and signs, distinct from the technician's inspection). This makes
-- that step enforceable at delivery: when a dealer turns on
-- settings.require_k208_licensee_certification, the customer disclosure cannot be
-- signed for a used/CPO vehicle until a signed K-208 is ALSO licensee-certified.
--
-- Default off — dealers opt in, so no in-flight deal is blocked by the upgrade.
-- Redefines get_ready_blocks_finalize preserving all existing behavior
-- (require_safety_inspection + k208_authority_roles) and adding the cert check.
-- ──────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_require IS TRUE OR v_require_cert IS TRUE THEN
    ...
    IF FOUND AND v_condition IN ('used','cpo','certified') THEN
      -- (a) require a signed (optionally role-authorized) K-208.
      ...
      -- (b) additionally require licensee certification of a signed K-208.
