...
-- ── Combined get-ready finalize gate: K-208 (with signer authority) OR
...
-- PRESERVES the k208_authority_roles logic from 20260629080000_k208_signer_
-- authority (this redefinition supersedes it, so it must carry it forward).
...
BEGIN
  ...
  -- K-208 half. Gated on require_safety_inspection; new cars exempt. When
  -- k208_authority_roles is set, the satisfying inspection must have been signed
  -- by a current, accepted tenant member holding one of those roles.
