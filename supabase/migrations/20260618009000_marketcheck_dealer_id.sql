-- MarketCheck dealer-scoped listings come from the syndication endpoint, which
-- is keyed on MarketCheck's internal dealer_id (the website domain only works in
-- analytics mode, which returns zero listings). The sync resolves dealer_id from
-- the domain automatically; this column lets a super admin paste it directly
-- when auto-resolution can't match a rooftop.
ALTER TABLE public.marketcheck_sync_config
  ADD COLUMN IF NOT EXISTS dealer_id text NOT NULL DEFAULT '';

-- Overload save_marketcheck_config to accept the dealer_id (the original 7-arg
-- version still resolves for older callers).
