...
serve(async (req) => {
  ...
  // ── SHADOW EVALUATION REMOVED (Gate 14G.5 SAFE RECOVERY RELEASE) ──────
  //
  // This build exists only as the qualified recovery target for
  // vehicle-enrich. It is the reviewed current function with ONE change: the
  // single shadow-trigger call site is deleted outright, not disabled behind
  // a flag, a secret, a budget or any other runtime switch. There is no code
  // path left from this function to market-valuation-write, so no shadow
  // valuation, comparable, snapshot, propagation, reservation or provider
  // request can originate here — structurally, not by configuration.
  //
  // Everything else is unchanged: constant-time credential comparison, the
  // certification resolver and conflict protection, the single-writer
  // boundary, and ordinary ingestion/enrichment behaviour.
