...
function Card({ title, children, className = "" }: { title?: string; children: ReactNode; className?: string }) {
  return (
    <section className={`rounded-[20px] border bg-white shadow-[0_10px_24px_rgba(15,23,42,0.06)] ${className}`} style={{ borderColor: line }}>
...
function Header({ qrUrl }: { qrUrl: string }) {
  ...
  return (
    <header className="grid grid-cols-[1.2fr_0.7fr_1.08fr] items-start gap-5">
      ...
      <div className="flex items-start gap-2 pt-1">
        ...
        <div><div className="text-[13px] font-black uppercase" style={{ color: ink }}>FTC Compliant</div><div className="text-[10px] leading-tight text-slate-500">Disclosures delivered digitally and in print.</div></div>
      </div>
...
function VehicleFallback() {
  return (
    <svg className="absolute left-[0.1in] top-[0.22in] drop-shadow-2xl" width="555" height="235" viewBox="0 0 555 235" fill="none" aria-label="vehicle image placeholder">
      <path d="M55 148c15-58 66-96 132-96h172c56 0 107 31 145 86l32 7c16 4 25 17 21 33l-5 18H25l7-27c4-14 10-20 23-21Z" fill="#263243" />
      <path d="M184 73h171c49 0 88 24 118 69H111c21-43 45-65 73-69Z" fill="#9aa8b8" />
      <path d="M208 89h85v50H134c19-31 45-48 74-50ZM318 89h42c36 0 67 18 95 50H318V89Z" fill="#f4f8fc" />
      <path d="M100 156h397" stroke="#f8fafc" strokeWidth="10" strokeLinecap="round" opacity="0.58" />
      <path d="M208 66h136" stroke="#f8fafc" strokeWidth="7" strokeLinecap="round" opacity="0.72" />
      <circle cx="140" cy="191" r="38" fill="#0f172a" />
      <circle cx="140" cy="191" r="18" fill="#e2e8f0" />
      <circle cx="443" cy="191" r="38" fill="#0f172a" />
      <circle cx="443" cy="191" r="18" fill="#e2e8f0" />
      <path d="M42 215h455" stroke="#94a3b8" strokeWidth="3" opacity="0.28" />
    </svg>
  );
}
...
function VehiclePhoto({ imageUrl }: { imageUrl?: string }) {
  return (
    <div className="relative h-[2.85in] overflow-hidden rounded-[26px] bg-gradient-to-br from-white via-slate-50 to-blue-50 shadow-[0_16px_36px_rgba(15,23,42,0.12)]">
      {imageUrl ? (
        <>
          <img src={imageUrl} alt="Vehicle" className="absolute inset-0 h-full w-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-white/5 via-transparent to-blue-900/10" />
        </>
      ) : (
        <>
          <div className="absolute inset-0" style={{ background: "radial-gradient(circle at 72% 40%, rgba(147,197,253,.52) 0, transparent 36%), linear-gradient(115deg, transparent 0 55%, rgba(37,99,235,.055) 55% 62%, transparent 62%)" }} />
          <VehicleFallback />
        </>
      )}
...
function PassportPanel() {
  return (
    <section className="rounded-[20px] p-4 text-white shadow-[0_14px_28px_rgba(7,31,63,0.18)]" style={{ background: `linear-gradient(145deg,${navy},#061326)` }}>
  ...
  return (
    <div className="bg-white shadow-2xl ring-1 ring-slate-200 print:shadow-none" style={{ width: "8.5in", height: "11in", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      <div className="flex h-full flex-col p-[0.22in]">
        ...
        <section className="mt-4 grid grid-cols-[0.84fr_1.16fr] gap-5">
          <div>
            <div className="text-[12px] font-black uppercase" style={{ color: blue }}>Vehicle Passport</div>
            <div className="mt-3 text-[28px] font-black leading-none" style={{ color: blue }}>2024</div>
            <h1 className="mt-2 text-[44px] font-black uppercase leading-[0.94] tracking-[-0.055em]" style={{ color: ink }}>{title}</h1>
            <div className="mt-4 rounded-[18px] p-4 text-white shadow-[0_12px_26px_rgba(37,99,235,0.22)]" style={{ background: `linear-gradient(145deg,${blue},#1d4ed8)` }}>
              <div className="text-[10px] font-black uppercase tracking-[0.18em] text-blue-100">Our Best Price</div>
              <div className="mt-1 text-[44px] font-black leading-none tracking-[-0.05em]">{money(vehicle.price)}</div>
              <div className="mt-1 text-[9px] font-semibold text-blue-100">Within market range · pricing updated today</div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-x-8 gap-y-3"><Meta label="Stock Number" value={vehicle.stock} /><Meta label="VIN" value={vehicle.vin} /><Meta label="Exterior" value={exterior} /><Meta label="Interior" value={interior} /></div>
          </div>
          <VehiclePhoto imageUrl={vehicle.imageUrl} />
        </section>

        <section className="mt-3 grid grid-cols-6 rounded-[18px] border bg-white py-3 shadow-[0_8px_22px_rgba(15,23,42,0.05)]" style={{ borderColor: line }}><Spec label="Mileage" value={mileage} sub="Miles" /><Spec label="Fuel Economy" value={`${fuel.city} / ${fuel.highway}`} sub="MPG City / Hwy" /><Spec label="Engine" value={engine} sub="Gasoline" /><Spec label="Drivetrain" value={drivetrain} sub="All Wheel Drive" /><Spec label="Transmission" value={trans} sub="Automatic" /><Spec label="Fuel Type" value="Gasoline" /></section>
