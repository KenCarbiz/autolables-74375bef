...
function VehiclePhoto({ imageUrl }: { imageUrl?: string }) {
  return (
    <div className="relative h-[2.36in] overflow-hidden rounded-[24px] bg-gradient-to-br from-white via-slate-50 to-blue-50">
      {imageUrl ? (
        <>
          <img src={imageUrl} alt="Vehicle" className="absolute inset-0 h-full w-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-r from-white/10 via-transparent to-blue-900/5" />
        </>
      ) : (
        <>
...
function Meta({ label, value }: { label: string; value: string }) { return <div><div className="text-[8.5px] font-black uppercase tracking-wide text-slate-500">{label}</div><div className="mt-0.5 text-[10.5px] font-black" style={{ color: ink }}>{value}</div></div>; }
function Spec({ label, value, sub }: { label: string; value: string; sub?: string }) { return <div className="flex items-center gap-3 border-r px-3 last:border-r-0" style={{ borderColor: line }}><div className="flex h-10 w-10 items-center justify-center rounded-full border text-[18px] font-black" style={{ borderColor: "#bfdbfe", color: ink }}>{label[0]}</div><div><div className="text-[8px] font-black uppercase tracking-wide text-slate-500">{label}</div><div className="text-[17px] font-black leading-none" style={{ color: ink }}>{value}</div>{sub ? <div className="text-[8px] text-slate-500">{sub}</div> : null}</div></div>; }
function ScoreGauge() { return <Card title="Vehicle Confidence Score"><div className="p-4 pt-3 text-center"><div className="relative mx-auto h-[1.34in] w-[1.55in]"><div className="absolute left-1/2 top-0 h-[1.35in] w-[1.35in] -translate-x-1/2 rounded-full border-[13px]" style={{ borderColor: green, borderBottomColor: "transparent", transform: "translateX(-50%) rotate(45deg)" }} /><div className="absolute inset-x-0 top-[0.42in] text-center"><div className="text-[46px] font-black leading-none" style={{ color: ink }}>93</div><div className="text-[12px] font-bold text-slate-500">/100</div></div></div><div className="mt-1 text-[14px] font-black uppercase" style={{ color: green }}>Excellent</div><div className="mx-auto mt-1 max-w-[1.7in] text-[9px] leading-tight text-slate-500">Our AI analyzes 50+ data points to rate this vehicle.</div></div></Card>; }
function PricePosition() { return <Card title="Price Position"><div className="p-4 pt-3 text-center"><div className="text-[19px] font-black uppercase" style={{ color: green }}>Great Value</div><div className="mt-1 text-[10px] font-semibold text-slate-500">$1,240 below market average</div><div className="relative mx-auto mt-4 h-[0.95in] w-[1.85in] overflow-hidden"><div className="absolute bottom-0 left-1/2 h-[1.65in] w-[1.65in] -translate-x-1/2 rounded-full border-[18px]" style={{ borderColor: "#d8e1ee", borderBottomColor: green, borderLeftColor: green, borderRightColor: "#7aa2ff", transform: "translateX(-50%) rotate(-45deg)" }} /><div className="absolute bottom-0 left-1/2 h-[0.75in] w-[2px] origin-bottom bg-slate-900" style={{ transform: "rotate(-3deg)" }} /></div><div className="grid grid-cols-3 text-[8px] text-slate-500"><span>Below</span><span>Market</span><span>Above</span></div></div></Card>; }
function PassportPanel() { return <section className="rounded-[18px] p-4 text-white" style={{ background: `linear-gradient(145deg,${navy},#061326)` }}><div className="text-[16px] font-black uppercase">Digital Vehicle Passport</div><p className="mt-2 text-[10px] leading-snug text-blue-100">Verified digital passport with blockchain-secured records.</p><div className="mt-3 space-y-1.5 text-[10.5px] font-semibold">{['Ownership History Verified', 'Title & Brand Check', 'Odometer Verified', 'Service & Maintenance Records', 'Inspection & Reconditioning'].map((x) => <div key={x}><span style={{ color: green }}>✓</span> {x}</div>)}</div><div className="mt-3 rounded-full bg-white px-4 py-2 text-center text-[10px] font-black" style={{ color: navy }}>View Full Passport Report</div></section>; }
function BenefitIcon({ icon, title, sub }: { icon: string; title: string; sub: string }) { return <div className="border-r px-3 text-center last:border-r-0" style={{ borderColor: line }}><div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full text-[19px] font-black" style={{ color: blue, background: "#eff6ff" }}>{icon}</div><div className="mt-1 text-[9.5px] font-black leading-tight" style={{ color: ink }}>{title}</div><div className="mt-0.5 text-[8px] leading-tight text-slate-500">{sub}</div></div>; }
function TrustScore() { return <Card title="Dealer Trust Score"><div className="grid grid-cols-[0.45fr_1fr] items-center gap-3 p-4 pt-2"><div className="flex h-16 w-16 items-center justify-center rounded-2xl text-[34px] font-black" style={{ color: blue, background: "#eff6ff" }}>★</div><div><div className="text-[38px] font-black leading-none" style={{ color: ink }}>4.9 <span className="text-[18px]" style={{ color: blue }}>★★★★★</span></div><div className="mt-1 text-[10px] text-slate-500">Based on 1,250+ verified reviews</div></div></div></Card>; }
...
function Pill({ children }: { children: ReactNode }) { return <span className="rounded-full border px-2.5 py-1 text-[8.5px] font-semibold leading-none" style={{ borderColor: line, color: ink }}>{children}</span>; }
  ...
  return (
    <div className="bg-white shadow-2xl ring-1 ring-slate-200 print:shadow-none" style={{ width: "8.5in", height: "11in", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      <div className="flex h-full flex-col p-[0.22in]">
        <Header qrUrl={qrUrl} />
        <section className="mt-4 grid grid-cols-[0.9fr_1.1fr] gap-5"><div><div className="text-[12px] font-black uppercase" style={{ color: blue }}>Vehicle Passport</div><div className="mt-3 text-[27px] font-black leading-none" style={{ color: blue }}>2024</div><h1 className="mt-2 text-[43px] font-black uppercase leading-[0.95] tracking-[-0.05em]" style={{ color: ink }}>{title}</h1><div className="mt-5 grid grid-cols-2 gap-x-8 gap-y-4"><Meta label="Stock Number" value={vehicle.stock} /><Meta label="VIN" value={vehicle.vin} /><Meta label="Exterior" value={exterior} /><Meta label="Interior" value={interior} /></div></div><VehiclePhoto imageUrl={vehicle.imageUrl} /></section>
        <section className="mt-3 grid grid-cols-6 rounded-[18px] border bg-white py-3" style={{ borderColor: line }}><Spec label="Mileage" value={mileage} sub="Miles" /><Spec label="Fuel Economy" value={`${fuel.city} / ${fuel.highway}`} sub="MPG City / Hwy" /><Spec label="Engine" value={engine} sub="Gasoline" /><Spec label="Drivetrain" value={drivetrain} sub="All Wheel Drive" /><Spec label="Transmission" value={trans} sub="Automatic" /><Spec label="Fuel Type" value="Gasoline" /></section>
        <section className="mt-3 grid grid-cols-[1fr_1fr_1.12fr] gap-3"><ScoreGauge /><PricePosition /><PassportPanel /></section>
        <section className="mt-3 grid grid-cols-[1.25fr_0.95fr] gap-3"><Card title="Ownership Benefits Included"><div className="grid grid-cols-5 p-3 pt-2"><BenefitIcon icon="S" title="10 Year / 100,000 Mile" sub="Powertrain warranty" /><BenefitIcon icon="M" title="2 Years" sub="Maintenance" /><BenefitIcon icon="R" title="24/7" sub="Roadside assistance" /><BenefitIcon icon="E" title="3 Day / 300 Mile" sub="Exchange policy" /><BenefitIcon icon="C" title="Free CARFAX" sub="Vehicle report" /></div></Card><TrustScore /></section>
