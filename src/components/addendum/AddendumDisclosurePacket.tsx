...
export function AddendumDisclosurePacket({
  ...
  return (
    <div className={`space-y-4 text-foreground ${className}`}>
      {/* Licensed seller identity */}
      {(dealer?.name || dealerAddr || dealer?.license_number) && (
        <div className="rounded-2xl border border-border bg-white p-4">
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Licensed seller</p>
          {dealer?.name && <p className="text-sm font-bold text-foreground mt-1">{dealer.name}</p>}
          {dealerAddr && <p className="text-[12px] text-muted-foreground">{dealerAddr}</p>}
        ...
        <details className="rounded-2xl border border-border bg-white p-4">
          ...
          <div className="mt-3 space-y-3">
            {products.map((p, i) => {
              ...
              return (
                <div key={p.id || i} className="border-t border-border pt-3 first:border-0 first:pt-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {typeof p.price === "number" && (
                      <p className="text-[13px] font-bold tabular-nums text-foreground shrink-0">{money(p.price)}</p>
                    )}
                  </div>
                  {p.subtitle && <p className="text-[12px] text-muted-foreground mt-1">{p.subtitle}</p>}
                  {benefit && <p className="text-[12px] text-foreground mt-1 whitespace-pre-line leading-snug">{benefit}</p>}
                  {p.disclosure && <p className="text-[11px] text-muted-foreground mt-1 whitespace-pre-line leading-snug">{p.disclosure}</p>}
                </div>
              );
            })}
          </div>
      ...
      <div className="rounded-2xl border border-border bg-white p-4">
        <p className="text-sm font-bold text-foreground">{pack.stateName} required disclosures &amp; consumer rights</p>
        <div className="mt-3 space-y-3">
          {blocks.map((b) => (
            <div key={b.id} className="border-t border-border pt-3 first:border-0 first:pt-0">
              <p className="text-[12px] font-bold text-foreground">
                {b.title}
                {b.required && <span className="ml-1 text-[9px] font-bold uppercase tracking-wider text-rose-600">Required</span>}
              </p>
              <p className="text-[12px] text-foreground mt-1 whitespace-pre-line leading-snug">{b.body}</p>
              {b.citation && <p className="text-[10px] text-muted-foreground mt-0.5">{b.citation}</p>}
            </div>
          ))}
              ...
              <>
                {c.salesContractClause && (
                  <div className="border-t border-border pt-3">
                    <p className="text-[12px] font-bold text-foreground">Sales contract notice</p>
                    <p className="text-[12px] text-foreground mt-1 whitespace-pre-line leading-snug">{c.salesContractClause}</p>
                  </div>
                )}
                {c.postSalePurchaseWindowDays > 0 && (
                  <div className="border-t border-border pt-3">
                    <p className="text-[12px] font-bold text-foreground">Post-sale purchase window</p>
                ...
                {c.recordRetentionYears > 0 && (
                  <div className="border-t border-border pt-3">
                    <p className="text-[12px] font-bold text-foreground">Record retention</p>
                    <p className="text-[12px] text-foreground mt-1 leading-snug">
