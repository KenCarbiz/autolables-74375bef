      ...
      <div className="grid gap-0 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <div className="space-y-4 p-5 sm:p-6">
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category} className="rounded-2xl border border-border bg-muted p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <h3 className="mt-2 text-lg font-black text-foreground">Approved visual proof</h3>
                </div>
                <div className="rounded-xl bg-white p-2 text-muted-foreground"><Image className="h-5 w-5" /></div>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                {items.map((photo, index) => (
                  <figure key={`${photo.photoUrl}-${index}`} className="overflow-hidden rounded-2xl border border-border bg-white shadow-sm">
                    ...
                    <figcaption className="p-3">
                      <p className="text-sm font-black text-foreground">{photo.caption || "Dealer-approved proof photo"}</p>
