...
import {
  ...
  fmtCurrency, sumLines, resolveConfig, resolvePrice,
} from "./labelData";

// ──────────────────────────────────────────────────────────────
// AddendumLabel — the 4.25in x 11in printed addendum. One component,
...
export default function AddendumLabel({ data }: { data: AddendumLabelData }) {
  const { tenant, vehicle, compact } = data;
  const config = resolveConfig(data.config);
  const price = resolvePrice(vehicle, config);
  // A section renders only when its config switch is on AND it has real rows.
  const installed = config.showInstalledProducts ? data.installed : [];
  const benefits = config.showIncludedBenefits ? data.benefits : [];
  const upgrades = config.showAvailableUpgrades ? data.upgrades : [];
  const installedValue = sumLines(installed);
  const upgradeValue = sumLines(upgrades);
  const anyDisclosure =
    config.disclosureMode !== "none" &&
    [...installed, ...benefits, ...upgrades].some((l) => l.disclosureRequired);

  return (
    <div
      className="addendum-label bg-white flex flex-col"
      style={{ width: "4.25in", height: "11in", boxSizing: "border-box", padding: "0.18in", color: NAVY, fontFamily: "Inter, Arial, sans-serif", overflow: "hidden" }}
    >
      ...
      <div className="flex-1 min-h-0 pt-1.5 space-y-1.5">
        {installed.length > 0 && (
          <div>
            ...
            <p className="text-slate-500" style={{ fontSize: "6px", padding: "1.5px 2px 0" }}>
              {config.installedProductsIncludedInTotal
                ? "Items above are installed on this vehicle and included in the displayed price."
                : "Items above are installed on this vehicle. See dealer for how they factor into the displayed price."}
            </p>
          </div>
        )}
        ...
        {upgrades.length > 0 && (
          <div>
            ...
            <p className="text-slate-500" style={{ fontSize: "6px", padding: "1.5px 2px 0" }}>
              {config.availableUpgradesIncludedInTotal
                ? "Optional upgrades are not installed. Pricing shown reflects them only where the dealer has documented their inclusion."
      ...
      <div className="rounded-lg border-2 flex items-stretch mt-1.5" style={{ borderColor: NAVY }}>
        ...
        <div className="px-2 py-1.5 border-l border-slate-200 text-right leading-tight" style={{ fontSize: "7px" }}>
          ...
          {upgradeValue > 0 && !config.availableUpgradesIncludedInTotal && <p className="text-slate-400" style={{ fontSize: "5.5px" }}>Available upgrades not included in total.</p>}
        </div>
      </div>
      ...
      {anyDisclosure && (
        <p className="text-slate-500" style={{ fontSize: "6px", marginTop: "3px" }}>
          {config.disclosureMode === "full"
            ? "*Marked items carry conditions. Available upgrades are optional and are not included in the vehicle total unless selected and documented. See dealer for complete details, or scan the Vehicle Passport QR code above for full disclosures."
      ...
      <div className="rounded-md mt-1.5 px-2 py-1.5 flex items-center justify-between" style={{ background: NAVY, color: "white" }}>
        ...
        {config.showFooterComplianceBadges && (
