    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="rounded-2xl border border-border bg-white p-5 space-y-5">
          ...
          <div>
            <label className="block text-xs font-semibold text-foreground mb-2">VIN detection</label>
            <div className="space-y-2">
              {([
                ...
                <button
                  ...
                  type="button"
                  onClick={() => setMode(m)}
                  className={`w-full text-left px-3 py-2 rounded-lg border transition ${mode === m ? "border-blue-500 bg-blue-50" : "border-border hover:border-border"}`}
                >
          ...
          {mode === "selector" && (
            <div>
              <label className="block text-xs font-semibold text-foreground mb-1">CSS selector</label>
              <input value={selector} onChange={(e) => setSelector(e.target.value)} className="w-full h-9 px-3 rounded-lg border border-border text-sm" placeholder=".vdp-vin" />
            </div>
          )}

          <div>
            <label className="block text-xs font-semibold text-foreground mb-1">Button label</label>
            <input value={label} onChange={(e) => setLabel(e.target.value)} className="w-full h-9 px-3 rounded-lg border border-border text-sm" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-foreground mb-1">Position</label>
              <select value={position} onChange={(e) => setPosition(e.target.value as Position)} className="w-full h-9 px-3 rounded-lg border border-border text-sm">
                <option value="right">Bottom right</option>
                <option value="left">Bottom left</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-foreground mb-1">Accent color</label>
              <div className="flex items-center gap-2">
                <input type="color" value={accent} onChange={(e) => setAccent(e.target.value)} className="h-9 w-12 rounded border border-border cursor-pointer" />
                <input value={accent} onChange={(e) => setAccent(e.target.value)} className="flex-1 h-9 px-3 rounded-lg border border-border text-sm font-mono uppercase" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-border bg-muted p-5 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">A mock VDP card with your live widget. Click the launcher.</p>
      ...
      <section className="rounded-2xl border border-border bg-white p-5 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-foreground">Install snippet</h2>
          </div>
        ...
        <div className="text-xs text-muted-foreground space-y-1.5">
          <p><strong>Where to paste:</strong> your VDP template, just before <code className="text-foreground bg-muted px-1 rounded">&lt;/body&gt;</code>.</p>
          {mode === "template" && (
            <p><strong>Template token:</strong> replace <code className="text-foreground bg-muted px-1 rounded">{"{{VIN}}"}</code> with your VDP's VIN merge tag (e.g. <code className="text-foreground bg-muted px-1 rounded">[[vehicle.vin]]</code>).</p>
          )}
          <p>The widget is async, lazy-loads the passport iframe on first open, and only surfaces the public <code className="text-foreground bg-muted px-1 rounded">/v/&lt;VIN&gt;</code> passport — no internal data is exposed.</p>
        </div>
      </section>
    </div>
  );
  ...
  return (
    <div className="relative rounded-xl border border-border bg-white overflow-hidden" style={{ height: 420 }}>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="text-[10px] text-muted-foreground mt-1">VIN {vin}</div>
      </div>
