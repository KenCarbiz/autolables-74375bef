...
const LAB_FIXTURES = {
  ...
  dodge: { label: "Dodge Durango R/T", data: dodgeDurangoFixture },
  buick: { label: "Buick Envision Avenir", data: buickEnvisionFixture },
  buickst: { label: "Envista Sport Touring", data: buickEnvistaFixture },
  buickenclave: { label: "Enclave Avenir", data: buickEnclaveFixture },
  dodgeev: { label: "Charger Daytona EV", data: dodgeChargerEvFixture },
  dodgephev: { label: "Hornet R/T PHEV", data: dodgeHornetPhevFixture },
  chrysler: { label: "Chrysler Pacifica", data: chryslerPacificaFixture },
  chryslerphev: { label: "Pacifica Plug-In Hybrid", data: chryslerPhevFixture },
  voyager: { label: "Chrysler Voyager", data: chryslerVoyagerFixture },
  landrover: { label: "Range Rover Sport", data: landRoverSportFixture },
  landroverphev: { label: "Range Rover SV LWB", data: landRoverPhevFixture },
  landroverdef: { label: "Defender 130", data: landRoverDefenderFixture },
      ...
      <div className="max-w-[1500px] mx-auto space-y-4">
        <div className="rounded-xl border border-border bg-white px-4 py-3 space-y-2">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <h1 className="text-base font-bold text-foreground">OEM Theme Laboratory</h1>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className={`h-8 px-2.5 rounded-md border text-[11px] font-bold ${grayscale ? "bg-slate-800 text-white border-slate-800" : "border-border hover:bg-muted"}`}
              >
                Grayscale
              </button>
              <button
                ...
                className={`h-8 px-2.5 rounded-md border text-[11px] font-bold ${actualSize ? "bg-blue-600 text-white border-blue-600" : "border-border hover:bg-muted"}`}
              >
                Actual print size
              </button>
              {result.model && result.model.pages.length > 1 && (
                <button
                  ...
                  className="h-8 px-2.5 rounded-md border border-border text-[11px] font-bold hover:bg-muted"
                >
          ...
          {profile && (
            <div className="flex items-center gap-2 flex-wrap text-[11px]">
              ...
              <span className="font-mono text-muted-foreground">{profile.themeVersion}</span>
              <span className="text-muted-foreground">family: <b>{profile.layoutFamily}</b></span>
