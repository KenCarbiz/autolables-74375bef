  ...
  return (
    <div className="bg-muted/30 min-h-screen py-6 print:bg-white print:py-0">
      ...
      <div className="no-print max-w-[820px] mx-auto mb-4 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">{items.length} vehicle{items.length === 1 ? "" : "s"} · windshield + key-fob service QR</p>
      ...
      {items.length === 0 ? (
        ...
        <div className="max-w-[820px] mx-auto bg-white p-6 shadow-premium print:shadow-none grid grid-cols-2 gap-4 text-[#0F172A]">
          {items.map((v) => {
            const url = `${window.location.origin}/ready/${v.token}`;
            return (
              <div key={v.vin} className="qr-card border border-slate-200 rounded-xl p-3 flex gap-3 items-center">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[9px] text-blue-700 font-bold uppercase tracking-wide mt-1">Scan for Get-Ready</p>
                </div>
              </div>
            );
          })}
