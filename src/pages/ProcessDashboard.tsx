    ...
    queryFn: async () => {
      ...
      const [openRes, recentRes, returnsRes] = await Promise.all([
        ...
        (supabase as any)
          ...
          .select("id, addendum_id, vin, signer_name, signed_at, content_hash")
            ...
            <div className="md:hidden space-y-2">
              {signings.recent.map(r => (
                <div key={r.id} className="rounded-xl border border-border bg-card p-3 flex items-center justify-between gap-3">
                  <button
                    onClick={() => r.addendum_id ? navigate(`/addendum?id=${r.addendum_id}`) : navigate(`/compliance?vin=${encodeURIComponent(r.vin || "")}`)}
                  ...
                  {signings.recent.map(r => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <button
                          onClick={(e) => { e.stopPropagation(); navigate(`/compliance?vin=${encodeURIComponent(r.vin || "")}`); }}
