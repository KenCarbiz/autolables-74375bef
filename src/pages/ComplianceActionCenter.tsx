            ...
            {loading ? (
              ...
              return (
                <tr key={row.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-3 py-3"><div className="flex max-w-[360px] flex-wrap gap-1">{issues.length ? issues.slice(0, 4).map((issue) => <span key={`${issue.key}-${issue.label}`} className="rounded-full border border-amber-200 bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">{issue.label || issue.key || "Issue"}</span>) : <span className="text-xs text-muted-foreground">None</span>}{issues.length > 4 ? <span className="rounded-full border border-border bg-muted px-2 py-1 text-[10px] font-bold text-muted-foreground">+{issues.length - 4}</span> : null}</div></td>
