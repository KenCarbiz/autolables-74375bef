...
describe("a real Harte INFINITI VDP", () => {
  ...
  it("reads one-owner from the page's own CARFAX sentence", () => {
    expect(detectOneOwnerBadge(VDP)).toBe(true);
  });

  it("reads clean title from CARFAX's 'no title brands'", () => {
    // MarketCheck omits carfax_clean_title from its search payload entirely, so
    // this sentence is the only source the platform has for the flag.
    expect(detectCleanTitleBadge(VDP)).toBe(true);
  });
